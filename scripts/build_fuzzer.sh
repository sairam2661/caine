...
#
# Usage:
#   ./scripts/build_fuzzer.sh           # unit test mode (fast, for dev)
#   ./scripts/build_fuzzer.sh --fuzz    # fuzzing mode (for actual fuzzing)
...
ninja -C "${BUILD_DIR}" -j "$(nproc)" opt_crash_fuzzer

echo ""
echo "=== Build complete ==="
echo "  Binary: ${BUILD_DIR}/fuzz_targets/opt_crash_fuzzer"
echo ""
...
if [[ "${FUZZING_MODE}" == "ON" ]]; then
    ...
    echo "  ${BUILD_DIR}/fuzz_targets/opt_crash_fuzzer"
    echo ""
    echo "To build in fuzzing mode:"
    echo "  ./scripts/build_fuzzer.sh --fuzz"
fi